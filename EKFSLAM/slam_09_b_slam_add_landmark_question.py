...
class ExtendedKalmanFilterSLAM:
    ...
    def predict(self, control):
        ...
        G_COV = zeros((3+2*N,3+2*N))
        I= eye(2*N)
        V_COV = zeros((3+2*N,3+2*N))
        V_COV[0:3,0:3] = R3

        G_COV[0:3,0:3] = G3
        G_COV[3:3+2*N,3:3+2*N]= I
        #print G_COV.shape
        self.covariance = dot(G_COV,dot(self.covariance,G_COV.T)) + V_COV

        # state' = g(state, control)
        R_state = self.g(self.state[0:3], control, self.robot_width)  
        self.state[0:3]= R_state
        self.state[3:3+2*N]= self.state[3:3+2*N]
    ...
    def add_landmark_to_state(self, initial_coords):
        ...
        N = self.number_of_landmarks
        new_state = zeros(3+2*N+2)
        new_state[0:3+2*N] = self.state
        new_state[3+2*N:3+2*N+2] = initial_coords
        self.state = new_state
        
        new_COV = zeros((3+2*N+2,3+2*N+2))
        new_COV[0:3+2*N,0:3+2*N] = self.covariance
        new_COV[3+2*N:3+2*N+2,3+2*N:3+2*N+2] = diag([1e10,1e10])
...
#        if N ==0:
#            return N  #N was assigned the number of lndmarks
#        else:
