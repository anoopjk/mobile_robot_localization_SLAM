...
class Particle:
    ...
    def H_Ql_jacobian_and_measurement_covariance_for_landmark(
        ...
        landmark_variance = np.dot(H, np.dot(self.landmark_covariances[landmark_number],H.T)) 
        predictedmeasurement_var = Qt_measurement_covariance
        Ql = landmark_variance + predictedmeasurement_var
        # Hints:
    ...
    def wl_likelihood_of_correspondence(self, measurement,
        ...
        # --->>> Insert your previous code here.
        z_hat = self.h_expected_measurement_for_landmark(landmark_number, scanner_displacement)
        delta_z = measurement - z_hat
        ...
        mahal = np.dot(delta_z,np.dot(invQl,delta_z.T))
        likelihood = k*exp(-0.5*mahal)
        # Hints:
    ...
    def initialize_new_landmark(self, measurement_in_scanner_system,
        ...
        landmark_covariance = np.dot(Hinv,np.dot(Qt_measurement_covariance,Hinv.T))
        ...
        self.landmark_covariances.append(landmark_covariance)
    ...
    def update_landmark(self, landmark_number, measurement,
        ...
        H,Ql = self.H_Ql_jacobian_and_measurement_covariance_for_landmark(landmark_number,
                             Qt_measurement_covariance, scanner_displacement)
        Qlinv = np.linalg.inv(Ql)
        K = np.dot(self.landmark_covariances[landmark_number],np.dot(H.T,Qlinv))
        predicted_measurement = self.h_expected_measurement_for_landmark(landmark_number, 
                                                        scanner_displacement)
        delta_z = measurement - predicted_measurement
        new_landmarkposition = self.landmark_positions[landmark_number] + \
                                   np.dot(K,delta_z)
        new_landmarkcovariance = np.dot((np.eye(2) - np.dot(K,H)), self.landmark_covariances[landmark_number])
        self.landmark_positions[landmark_number] = new_landmarkposition
        self.landmark_covariances[landmark_number] = new_landmarkcovariance
    ...
    def update_particle(self, measurement, measurement_in_scanner_system,
        ...
        if (not likelihoods) or \
           ...
           self.initialize_new_landmark( measurement_in_scanner_system,
                                Qt_measurement_covariance,
                                scanner_displacement)  #landmarks will be incremented
        ...
        # Else update the particle's EKF for the corresponding particle.
           ...
           # w = max(likelihoods)
            ...
            maximum =0
            for i,value in enumerate(likelihoods):
                if value > maximum:
                    maximum = value
                    index = i
            #index, w = max(enumerate(likelihoods), key = operator.itemgetter(1))
    ...
    def decrement_visible_landmark_counters(self, scanner_displacement):
        ...
        # Hints:
        minb,maxb =  LegoLogfile.min_max_bearing() 
        for i in range(0,len(self.landmark_positions)):
            distance,  bearing = self.h_expected_measurement_for_landmark( i,
                                                scanner_displacement)
            #print bearing
            if(bearing < maxb)and(bearing > minb):
    ...
    def remove_spurious_landmarks(self):
        ...
        # --->>> Insert your new code here.
        neg_list = []
        for i in range(0,len(self.landmark_counters)):
            if self.landmark_counters[i] < 0:
                neg_list.append(i)
                
                
        self.landmark_positions = [i for j, i in enumerate(self.landmark_positions) if j not in neg_list]
        self.landmark_covariances = [i for j, i in enumerate(self.landmark_covariances) if j not in neg_list]
